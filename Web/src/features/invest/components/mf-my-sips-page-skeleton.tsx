...
function MfMySipsFilterBarSkeleton() {
  return (
...
function MfMySipsTableRowSkeleton() {
  return (
...
export function MfMySipsTableSkeleton({ rows = 6 }: { rows?: number }) {
  return (
    <div className="flex flex-col overflow-hidden" aria-hidden="true">
      ...
      <div>
        {Array.from({ length: rows }).map((_, index) => (
          <MfMySipsTableRowSkeleton key={index} />
        ))}
      </div>
    </div>
  );
...
export function MfMySipsPageSkeleton() {
  return (
    <div className="space-y-6" aria-hidden="true">
      <MfMySipsFilterBarSkeleton />
