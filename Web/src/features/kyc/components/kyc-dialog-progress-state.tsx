  ...
  if (variant === "loading") {
    return (
      <div
        ...
        aria-busy="true"
      >
        <div className="flex size-16 items-center justify-center rounded-full bg-primary/5 ring-1 ring-inset ring-primary/15">
          <Loader2 className="size-7 animate-spin text-primary" strokeWidth={2.25} aria-hidden />
        </div>
        <div className="space-y-1">
          <p className="text-compact font-medium text-foreground">{copy.kyc.bootstrapLoadingTitle}</p>
          <p className="text-caption text-muted-foreground">{copy.kyc.loading}</p>
        </div>
      </div>
    );
  }
