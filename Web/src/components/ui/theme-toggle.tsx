...
export function ThemeToggle({ theme, onThemeChange, className }: ThemeToggleProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "relative inline-flex h-9 w-[4.5rem] shrink-0 cursor-pointer items-center rounded-full bg-transparent p-1 transition-colors hover:opacity-90 focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none",
        className
      )}
    >
      <span
