...
type SettingsAddBankAccountFormProps = {
  onSuccess: (account: InvestorBankAccount) => void;
  onCancel: () => void;
};

export function SettingsAddBankAccountForm({ onSuccess, onCancel }: SettingsAddBankAccountFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-[var(--radius-card)] border border-border bg-muted/10 p-4 sm:p-5">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="settings-bank-account-number">{copy.kyc.bank.fields.accountNumber}</Label>
          <Input
        ...
        <div className="space-y-2">
          <Label htmlFor="settings-bank-ifsc">{copy.kyc.bank.fields.ifscCode}</Label>
          <Input
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <Button type="button" variant="ghost" onClick={onCancel} disabled={isProcessing || proofUploading}>
          {copy.settings.bankAccounts.cancelAdd}
        </Button>
        <Button
          type="submit"
          disabled={isProcessing || proofUploading || isComplete}
