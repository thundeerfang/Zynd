...
import { ChangePasswordHeroImage } from "@/components/dashboard/settings/change-password-hero-image";
import { BrandDialog } from "@/components/ui/brand-dialog";
import { Button } from "@/components/ui/button";
import { copy } from "@/shared/config/copy";
import { useResetWhenDialogOpens } from "@/hooks/use-reset-when-dialog-opens";

type ChangePasswordVerifyDialogProps = {
  open: boolean;
...
export function ChangePasswordVerifyDialog({
  open,
  onOpenChange,
  loading = false,
  error = "",
  onErrorChange,
  onSubmit,
}: ChangePasswordVerifyDialogProps) {
      ...
      <form className="space-y-4 px-5 pb-5 pt-1.5" onSubmit={handleSubmit}>
        ...
        <AuthSubmitFooter className="pt-0">
          <Button type="submit" className="w-full" disabled={loading || !canSubmit}>
            {loading ? copy.settings.changePasswordSubmitting : copy.settings.changePasswordSubmit}
          </Button>
        </AuthSubmitFooter>
      </form>
    </BrandDialog>
...
/** @deprecated Use ChangePasswordVerifyDialog — kept for existing imports during transition */
export const ChangePasswordDialog = ChangePasswordVerifyDialog;
