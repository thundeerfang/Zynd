...
export const ORDER_SORT_OPTIONS: AdminSelectOption[] = [
...
export const ORDER_STATUS_OPTIONS: AdminSelectOption[] = [
...
export function MfTransactionOrdersPanel({
  ...
  search: searchProp,
  onSearchChange,
  orderStatusFilter: orderStatusFilterProp,
  onOrderStatusFilterChange,
  orderSort: orderSortProp,
  onOrderSortChange,
  refreshKey,
}: {
  canRead: boolean;
  ...
  onOrderSortChange?: (value: OrderSortKey) => void;
  refreshKey?: number;
}) {
  ...
  const [internalOrderStatusFilter, setInternalOrderStatusFilter] = useState(ALL);
  const [internalOrderSort, setInternalOrderSort] = useState<OrderSortKey>("recent");
  const search = onSearchChange ? (searchProp ?? "") : internalSearch;
  const setSearch = onSearchChange ?? setInternalSearch;
  const orderStatusFilter = onOrderStatusFilterChange
    ? (orderStatusFilterProp ?? ALL)
    : internalOrderStatusFilter;
  const setOrderStatusFilter = onOrderStatusFilterChange ?? setInternalOrderStatusFilter;
  const orderSort = onOrderSortChange ? (orderSortProp ?? "recent") : internalOrderSort;
  const setOrderSort = onOrderSortChange ?? setInternalOrderSort;
