...
import { DISTRIBUTOR_DEMO_AGENTS } from "@/lib/distributor-agents";
import { env } from "@/lib/env";
import { DISTRIBUTOR_LOGIN_CONTENT_CLASS } from "@/lib/distributor-layout";
...
export function DistributorLoginCard() {
  ...
  return (
    <Card className="w-full max-w-md shadow-zynd-mid">
      <CardContent className={DISTRIBUTOR_LOGIN_CONTENT_CLASS}>
          ...
          <div>
            ...
            <p className="mt-1 text-caption text-muted-foreground">
              {env.useBackendClients
                ? "Sign in with your distributor console account (dev: distributor@zynd.com / 12345678)."
                : "Distributor console — frontend demo with dummy data"}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <FieldGroup>
            <Field>
              <FieldLabel htmlFor="distributor-email">Email</FieldLabel>
              <Input
                ...
                placeholder="riya@zynd.distributor"
              />
            </Field>
            <Field>
              <FieldLabel htmlFor="distributor-password">Password</FieldLabel>
              <Input
        ...
        <div className="rounded-[var(--radius-card)] border border-border bg-muted/30 px-3 py-3">
          ...
          <ul className="mt-2 space-y-1.5 text-caption text-muted-foreground">
            {DISTRIBUTOR_DEMO_AGENTS.map((agent) => (
              <li key={agent.id}>
                <button
                  type="button"
                ...
                >
                  ...
                  {agent.role === "branch_manager" ? " · branch manager" : ""}
                </button>
              </li>
            ))}
          </ul>
